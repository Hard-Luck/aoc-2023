...
const part2 = (rawInput) => {
  ...
  do {
    ...
    console.log({
      ...
      firstDash,
      startD,
      value: getValueFromMap(location, map),
    });
    direction = step[1];
...
function clearNeighbours(point, map, opts) {
  if (getValueFromMap(point, map) === true) {
    exclude(point, map);
...
function checkEndOfLoop(direction, next) {
  const directionKey = `${direction[0]},${direction[1]}`;
  return legalMoves[directionKey].includes(next);
}
const part1 = () => {};
...
function clearNeighbours(point, map) {
  if (getValueFromMap(point, map) !== true) return;
  const neighbours = getNeighbours(point, map);
  console.log(neighbours);
  while (neighbours.length) {
    const current = neighbours.pop();
    if (getValueFromMap(current, map) !== true) neighbours.push(current);
    clearNeighbours(current, map);
  }
}
function updatePosition(point, map, value) {
  map[point[0]][point[1]] = value;
